...
const fixture=http.createServer((req,res)=>{res.setHeader("Content-Type","text/html");const name=req.url.includes("red")?"V34 Red":"V34 Blue";res.end(`<!doctype html><html><body><input id="username" value="${name}"><div id="homepage"></div><aside id="nc-11v11-announcement" style="display:block">11 VS 11</aside><script>setInterval(()=>{document.getElementById("nc-11v11-announcement").style.display="block"},100)</script></body></html>`);});
...
assert.match(releasedSource,/VERSION="0\.1\.34"/);
...
try{
  ...
  await page.screenshot({path:new URL("visual-test-v34.png",import.meta.url).pathname.replace(/^\/(.:)/,"$1")});
  console.log("PASS real Edge: v0.1.34 defaults to the Frankfurt server, hides the hosted 4v4 teaser, and preserves active-corner camera controls");
}finally{
