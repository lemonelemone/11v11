...
try{
  ...
  assert.equal((await fetch("http://127.0.0.1:18133/test/setup-wall")).ok,true);await next(blue,"notice",m=>m.text==="Wall contest released");const released=await next(blue,"state",m=>Math.hypot(m.b[2],m.b[3])>.1);assert.ok(Math.hypot(released.b[2],released.b[3])>.1,"strict wall contest release moves the ball");blue.ws.close();red.ws.close();console.log("PASS v0.1.34 in-pitch corner placement, larger circle, corner/goal-kick alternation, doubled corner touch and wall release");
}finally{child.kill();}
