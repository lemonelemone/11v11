...
try{
  ...
  for(let i=0;i<22;i++){
    ...
    assert.equal(c.m.t,"welcome");assert.equal(c.m.protocol,29);assert.equal(c.m.mode.BUILD,"0.1.33");
    players.push(c);
  }
  ...
  console.log("PASS: v0.1.33 two-minute default, core rules, five-player defensive-fifth cap, team chat, boost, penalties and replays");
} finally {child.kill();}
