...
try{
  ...
  const keepAlive=setInterval(()=>active.ws.send(JSON.stringify({t:"input",x:1,y:0,boost:false})),200);
  const deadline=Date.now()+4000;while(Date.now()<deadline&&!idle.messages.some(m=>m.t==="returnHome"))await wait(20);clearInterval(keepAlive);
  ...
  console.log("PASS inactivity kick: modal event sent, roster broadcast, player removed server-side");
}finally{child.kill();}
