...
try{
  ...
  await page.screenshot({path:new URL("visual-test-v31.png",import.meta.url).pathname.replace(/^\/(.:)/,"$1")});await page.locator("#nc11-admin").click();await page.waitForFunction(()=>getComputedStyle(document.querySelector("#nc11-admin-panel")).display==="block");await page.screenshot({path:new URL("visual-test-v31-admin.png",import.meta.url).pathname.replace(/^\/(.:)/,"$1")});await page.locator("#nc11-admin-close").click();
  await page.keyboard.press("c");
  await page.mouse.move(1200,450);await page.mouse.down();await page.waitForTimeout(250);
  await page.screenshot({path:new URL("visual-test-v31-follow-boosting.png",import.meta.url).pathname.replace(/^\/(.:)/,"$1")});await page.mouse.up();padProbe.send(JSON.stringify({t:"testRandomEvent"}));await page.waitForFunction(()=>getComputedStyle(document.querySelector("#nc11-event-overlay")).display==="block");assert.equal(await page.locator("#nc11-event-overlay").evaluate(e=>getComputedStyle(e).pointerEvents),"none");await page.screenshot({path:new URL("visual-test-v31-random-event.png",import.meta.url).pathname.replace(/^\/(.:)/,"$1")});const boostBefore=parseFloat(await page.locator("#nc11-boost span").evaluate(e=>e.style.width));await page.mouse.move(1200,450);await page.mouse.down();await page.waitForTimeout(220);await page.mouse.up();await page.waitForFunction(before=>parseFloat(document.querySelector("#nc11-boost span").style.width)<before,boostBefore);await page.waitForFunction(()=>getComputedStyle(document.querySelector("#nc11-event-overlay")).display==="none",null,{timeout:2000});padProbe.send(JSON.stringify({t:"testBallPosition",x:13,y:42.1875,vx:-8,vy:0}));await page.waitForFunction(()=>document.documentElement.dataset.nc11GoalExplosion==="active",null,{timeout:3000});await page.waitForTimeout(180);await page.screenshot({path:new URL("visual-test-v31-goal-explosion.png",import.meta.url).pathname.replace(/^\/(.:)/,"$1")});padProbe.close();
