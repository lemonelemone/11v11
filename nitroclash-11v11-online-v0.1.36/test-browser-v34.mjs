...
const fixture=http.createServer((req,res)=>{res.setHeader("Content-Type","text/html");const name=req.url.includes("red")?"V36 Red":"V36 Blue";res.end(`<!doctype html><html><body><input id="username" value="${name}"><div id="homepage"></div><aside id="nc-11v11-announcement" style="display:block">11 VS 11</aside><script>setInterval(()=>{document.getElementById("nc-11v11-announcement").style.display="block"},100)</script></body></html>`);});
...
assert.match(releasedSource,/VERSION="0\.1\.36"/);
...
try{
  ...
  assert.match(await page.evaluate(()=>document.documentElement.dataset.nc11Interpolation),/^tick-(?:2[5-9]|[34][0-9]|50)ms$/);
  ...
  await page.screenshot({path:new URL("visual-test-v36-corner.png",import.meta.url).pathname.replace(/^\/(.:)/,"$1")});
  assert.equal((await fetch("http://127.0.0.1:18131/test/setup-goal")).ok,true);await page.waitForSelector("#nc11-goal-card",{state:"visible",timeout:3000});assert.match(await page.locator("#nc11-goal-card").innerText(),/GOAL SCORER:[\s\S]*SHOT SPEED:[\s\S]*ASSIST:/);await page.screenshot({path:new URL("visual-test-v36-goal.png",import.meta.url).pathname.replace(/^\/(.:)/,"$1")});
  console.log("PASS real Edge: v0.1.36 uses low-latency tick interpolation and preserves active-corner camera controls");
}finally{
