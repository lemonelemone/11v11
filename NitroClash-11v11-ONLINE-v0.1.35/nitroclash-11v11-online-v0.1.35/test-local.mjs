...
try{
  ...
  for(let i=0;i<22;i++){
    ...
    assert.equal(c.m.t,"welcome");assert.equal(c.m.protocol,30);assert.equal(c.m.mode.BUILD,"0.1.35");
    players.push(c);
  }
  ...
  assert.equal(mode.PENALTY_INITIAL_ROUNDS,5);assert.equal(mode.PENALTY_BOX_LINE_X,116.8945);assert.equal(mode.PENALTY_BALL_X,116.8945);assert.equal(mode.PENALTY_TAKER_X,100.1055);assert.equal(mode.PENALTY_BALL_SPEED_MULTIPLIER,1.4);assert.equal(mode.PENALTY_SHOT_MS,1000);assert.equal(mode.PENALTY_UNLIMITED_BOOST,true);
  ...
  paceState=pace.messages.filter(m=>m.t==="state").at(-1);paceSpeed=Math.hypot(paceState.p[pace.m.slot][4],paceState.p[pace.m.slot][5]);assert.ok(paceSpeed>=22&&paceSpeed<=26,`boost pace ${paceSpeed}`);pace.ws.send(JSON.stringify({t:"input",x:1,y:0,boost:true,brake:true}));await wait(100);const brakeEnergy=pace.messages.filter(m=>m.t==="state").at(-1).p[pace.m.slot][6];await wait(300);const braked=pace.messages.filter(m=>m.t==="state").at(-1).p[pace.m.slot];assert.ok(braked[6]>=brakeEnergy,"holding boost while braking does not consume boost");assert.equal(braked[9],0,"braking suppresses the visible boost state");pace.ws.close();
  ...
  const goalProbe=await connect({party:"GOAL23",clientId:"goal-probe",name:"GoalProbe"});await wait(3100);const goal=goalProbe.next("goal"),goalReplay=goalProbe.next("goalReplay",8000);goalProbe.ws.send(JSON.stringify({t:"testBallPosition",x:137.0,y:42.1875,vx:8,vy:0}));const goalEvent=await goal;assert.equal(goalEvent.team,0,"ball scores just inside the clean stock goal instead of requiring a deep net push");assert.equal(goalEvent.celebrationMs,5000);const celebration=await waitState(goalProbe,s=>s.phase==="goalCelebration");assert.equal(celebration.p[goalProbe.m.slot][6],100,"the scoring team receives unlimited celebration boost");const replayClip=await goalReplay;assert.equal(replayClip.durationMs,6000);assert.ok(replayClip.frames.length>0,"goal replay is queued after the celebration and includes recorded frames");goalProbe.ws.close();
  ...
  console.log("PASS: v0.1.35 core rules, celebration, five-player defensive-fifth cap, team chat, boost, penalties and replays");
} finally {child.kill();}
